...
class Target_Adaptive_Identify_Feature_Feeding_Block(nn.Module):
    ...
    def forward(self, z_id,z_target):
        out = self.fc(z_id)
...
class Operation_Unit(nn.Module):
    def __init__(self, channel, identity_feature_out_dim, identity_feature_in_dim=512, act_out=True):
        ...
        self.IFF = Target_Adaptive_Identify_Feature_Feeding_Block(self.id_feature_out_dim, z_id_size=self.id_feature_in_dim)
...
class Encoder_noBNIN(nn.Module):
    def __init__(self, id_feature_dim):
        ...
        self.fusion_module = nn.ModuleDict(
            {f'fusion_layer_{i}': Cross_Adaptive_Identity_Injdection_Block(1024, 2048, 512)
             for i in range(6)})
